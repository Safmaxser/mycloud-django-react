import { useEffect } from 'react';
import { AlertCircle, FolderOpen, SearchX } from 'lucide-react';

import { useAppDispatch, useAppSelector } from '../../../store/hooks';
import { clearItems, fetchFiles, setPage } from '../../../store/slices/storageSlice';

import { FileTable } from './FileTable';
import { LoadingState } from '../../UI/LoadingState';
import { EmptyState } from '../../UI/EmptyState';
import { Pagination } from '../../UI/Pagination';
import { SearchResultsInfo } from '../../UI/SearchResultsInfo';

interface FileListSectionProps {
  /** ID пользователя для админ-режима. Если undefined — загружаются личные файлы. */
  userId: string | undefined;
}

/**
 * Основная контентная секция со списком файлов.
 * Управляет загрузкой данных, пагинацией и переключением состояний (загрузка, ошибка, пустой список).
 */
export function FileListSection({ userId }: FileListSectionProps) {
  const dispatch = useAppDispatch();
  const { files, loading, error, totalCount, page, ordering, search } = useAppSelector(
    (state) => state.storage,
  );

  const isAdminView = Boolean(userId);

  // Загрузка данных при изменении фильтров или ID пользователя
  useEffect(() => {
    const promise = dispatch(fetchFiles({ userId }));
    return () => {
      promise.abort();
    };
  }, [userId, page, ordering, search, dispatch]);

  // Очистка данных при размонтировании компонента
  useEffect(() => {
    return () => {
      dispatch(clearItems());
    };
  }, [dispatch]);

  const handlePageChange = (newPage: number) => {
    dispatch(setPage(newPage));
  };

  if (loading && files.length === 0) {
    return <LoadingState title="Загрузка данных файлов..." />;
  }

  if (error && !loading && files.length === 0) {
    return (
      <div className="flex h-full flex-col p-7">
        <EmptyState
          title="Произошла ошибка"
          description={error}
          icon={AlertCircle}
          action={
            <button
              onClick={() => dispatch(fetchFiles({ userId }))}
              className="mt-4 cursor-pointer rounded-2xl bg-gray-100 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-200 hover:text-gray-700"
            >
              Попробовать снова
            </button>
          }
        />
      </div>
    );
  }

  if (files.length === 0 && search === '' && !loading) {
    return (
      <div className="flex h-full flex-col p-7">
        <EmptyState
          title={isAdminView ? 'Хранилище пусто' : 'Ваше облако пусто'}
          description={
            isAdminView
              ? 'Этот пользователь еще не загрузил ни одного файла.'
              : 'Начните работу, загрузив свой первый файл в облако.'
          }
          icon={FolderOpen}
        />
      </div>
    );
  }

  if (files.length === 0 && search !== '' && !loading) {
    return (
      <div className="relative flex h-full flex-col p-7">
        <SearchResultsInfo search={search} count={totalCount} />
        <EmptyState
          title="Ничего не найдено"
          description={`По запросу «${search}» совпадений нет. Попробуйте изменить фильтры.`}
          icon={SearchX}
        />
      </div>
    );
  }

  return (
    <div className="custom-scrollbar relative flex h-full flex-col gap-6 overflow-y-auto p-7">
      <SearchResultsInfo search={search} count={totalCount} />
      <div className="flex-1">
        <FileTable files={files} />
      </div>
      <div className="sticky bottom-0 rounded-2xl border-b border-gray-100 bg-gray-50/80 py-2 backdrop-blur-md">
        <Pagination current={page} total={totalCount} onChange={handlePageChange} />
      </div>
    </div>
  );
}
